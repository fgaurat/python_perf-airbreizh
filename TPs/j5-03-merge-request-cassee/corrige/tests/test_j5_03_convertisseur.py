...
# --- Non-régression MR !418 -----------------------------------------------------------------


@pytest.mark.parametrize(
    ("valeur", "de", "vers", "attendu"),
    [
        pytest.param(1, "in", "mm", 25.4, id="pouce_vers_mm"),
        pytest.param(1, "mm", "in", 0.0394, id="mm_vers_pouce"),
        pytest.param(12, "in", "ft", 1.0, id="douze_pouces_font_un_pied"),
        pytest.param(1, "ft", "in", 12.0, id="un_pied_fait_douze_pouces"),
    ],
)
def test_l_arrondi_ne_porte_que_sur_le_resultat_final(valeur, de, vers, attendu):
    ...
    assert convertir_longueur(valeur, de, vers) == pytest.approx(attendu, rel=1e-3)
