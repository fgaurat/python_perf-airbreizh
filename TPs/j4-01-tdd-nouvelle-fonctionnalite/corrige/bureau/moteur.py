...
from bureau.historique import CAPACITE_DEFAUT, Historique
...
class Calculatrice:
    ...
    def __init__(
        self, precision=PRECISION_DEFAUT, mode="degres", journal=None, capacite=CAPACITE_DEFAUT
    ):
        ...
        self.nb_calculs = 0
        self.historique = Historique(capacite)  # ← la greffe (1/3)
    ...
    def calculer(self, expression, verbose=False):
        ...
        self.nb_calculs += 1
        self.historique.enregistrer(expression, resultat)  # ← la greffe (2/3)
    ...
    def annuler(self):
        """Revient au résultat précédent. ← la greffe (3/3)"""
        self.historique.annuler()
        courant = self.historique.courant
        self.derniere_valeur = courant.resultat if courant else 0.0
        return self.derniere_valeur
